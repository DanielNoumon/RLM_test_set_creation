...
class REPLEnv:
    ...
    def load_context(self, context_json: Optional[dict | list] = None, context_str: Optional[str] = None):
        ...
        if context_json is not None:
            ...
            # If context is a list of dicts with document structure,
            # inject helper functions for iterative document browsing
            if isinstance(context_json, list) and len(context_json) > 0 and isinstance(context_json[0], dict):
                helpers_code = '''
def list_documents():
    """List all available documents with index, filename, and size."""
    for i, doc in enumerate(context):
        if isinstance(doc, dict):
            name = doc.get("filename", f"document_{i}")
            size = len(doc.get("content", str(doc)))
            doc_type = doc.get("type", "unknown")
            print(f"[{i}] {name} ({doc_type}, {size} chars)")
        else:
            print(f"[{i}] (raw entry, {len(str(doc))} chars)")

def get_document(index):
    """Get the full content of a document by its index."""
    if index < 0 or index >= len(context):
        return f"Error: index {index} out of range (0-{len(context)-1})"
    doc = context[index]
    if isinstance(doc, dict):
...
def search_documents(keyword):
    """Search all documents for a keyword. Returns list of (index, filename, snippet)."""
    ...
    for i, doc in enumerate(context):
        ...
        if keyword_lower in content.lower():
            ...
            results.append((i, name, snippet))
            print(f"[{i}] {name}: ...{snippet}...")
    if not results:
        print(f"No documents contain '{keyword}'")
    return results
'''
                self.code_execution(helpers_code)
